...
public:
   ...
   * **/
  template <class M>
  void registerCallback(void (M::*fp)(const boost::shared_ptr<T const>&), M* obj, uint32_t queue_size = 10);
...
template <typename T>
template <typename M>
inline void Subscriber<T>::registerCallback(void (M::*fp)(const boost::shared_ptr<T const>&), M* obj,
