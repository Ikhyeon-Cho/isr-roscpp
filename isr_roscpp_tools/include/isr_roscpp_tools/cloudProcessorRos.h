...
namespace isr::navigation
{
    ...
    private:
        ...
        float pointDistance(const T &p) { return std::sqrt(p.x * p.x + p.y * p.y + p.z * p.z); };
    };
    ...
    template <class T>
    bool CloudProcessorROS<T>::transformPointCloudTo(const std::string &_target_frame)
    {
        ...
        if (source_frame == _target_frame)
            return true;

        geometry_msgs::TransformStamped transform_stamped;
        ros::Time timestampRos = pcl_conversions::fromPCL(pclCloud_->header.stamp);
        if (!transform_handler_.getTransform(transform_stamped, _target_frame, source_frame, timestampRos))
            return false;

        Eigen::Affine3d transform_eigen = transform_handler_.toEigenMatrix(transform_stamped.transform);

        pCloudPtr transformedCloud = boost::make_shared<pCloud>();
        transformedCloud->header = pclCloud_->header;
        transformedCloud->header.frame_id = _target_frame;
    ...
    template <class T>
    void CloudProcessorROS<T>::filterCloudByAxis(const std::string &_axis, double _minRange, double _maxRange, bool _negative)
    {
        if (cloudIsEmpty())
            return;
